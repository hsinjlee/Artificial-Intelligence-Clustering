...
public class Layer {
	...
	public float[] run(float[] inputArray) {
		...
		for (int i = 0; i < output.length; i++) {
			...
			output[i] = com.balazsholczer.neuralnet.ActivationFunction.sigmoid(output[i]);
			offset += input.length;
		}
	...
	public float[] train(float[] error, float learningRate, float momentum) {
		...
		for (int i = 0; i < output.length; i++) {
			...
			float delta = error[i] * com.balazsholczer.neuralnet.ActivationFunction.dSigmoid(output[i]); // because the output is the sigmoid(x) !!!
